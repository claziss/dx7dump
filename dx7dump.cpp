...
void VolcaFormat(const DX7Sysex *sysex, const char *filename)
{
  unsigned int rangeL = 0, rangeH = 32;
  ...
  for (unsigned voiceNum = rangeL; voiceNum < rangeH; ++voiceNum)
    {
      const VoicePacked *voice = &(sysex->voices[voiceNum]);
      ...
      if (!longListing)
	{
      ...
      printf("Algorithm: %u\n", voice->algorithm + 1);
      printf("Pitch Envelope Generator\n");
      printf("  Rate 1  (Ptr1): %u\n", voice->pitchEGR1);
      printf("  Rate 2  (Ptr2): %u\n", voice->pitchEGR2);
      printf("  Rate 3  (Ptr3): %u\n", voice->pitchEGR3);
      printf("  Rate 4  (Ptr4): %u\n", voice->pitchEGR4);
      printf("  Level 1 (PtL1): %u\n", voice->pitchEGL1);
      printf("  Level 2 (PtL2): %u\n", voice->pitchEGL2);
      printf("  Level 3 (PtL3): %u\n", voice->pitchEGL3);
      printf("  Level 4 (PtL4): %u\n", voice->pitchEGL4);

      printf("Feedback (Fdbk): %u\n", voice->feedback);
      printf("Oscillator Key Sync (OKS): %s\n", OnOff(voice->oscKeySync));

      printf("LFO\n");
      printf("  Delay    (LFOd): %u\n", voice->lfoDelay);
      printf("  AM Depth (LANd): %u\n", voice->lfoAMDepth);
      printf("  Sync     (LFOK): %s\n", OnOff(voice->lfoSync));
      printf("  Wave     (LFOU): %s\n", LFOWave(voice->lfoWave));
      printf("  Speed    (LFOr): %u\n", voice->lfoSpeed);
      printf("  Pitch Mod Depth (LPNd): %u\n", voice->lfoPitchModDepth);

      printf("  Pitch Modulation Sensitivity (NSP): %u\n",
	     voice->lfoPitchModSensitivity);

      printf("Transpose (trSP): %s (%d)\n", Transpose(voice->transpose).c_str(),
	     voice->transpose);
      ...
      for (unsigned i = 0; i < 6; ++i)
	{
	  ...
	  printf("  Envelope Generator\n");
	  printf("    Rate 1  (EGr1): %u\n", op.EG_R1);
	  printf("    Rate 2  (EGr2): %u\n", op.EG_R2);
	  printf("    Rate 3  (EGr3): %u\n", op.EG_R3);
	  printf("    Rate 4  (EGr4): %u\n", op.EG_R4);
	  printf("    Level 1 (EGL1): %u\n", op.EG_L1);
	  printf("    Level 2 (EGL2): %u\n", op.EG_L2);
	  printf("    Level 3 (EGL3): %u\n", op.EG_L3);
	  printf("    Level 4 (EGL4): %u\n", op.EG_L4);
	  printf("  Keyboard Level Scaling\n");
	  printf("    Breakpoint (LSbP): %s (%d)\n",
		 Breakpoint(op.levelScalingBreakPoint).c_str(),
		 op.levelScalingBreakPoint);
	  printf("    Left Depth  (LSLd): %u\n", op.scaleLeftDepth);
	  printf("    Right Depth (LSrd): %u\n", op.scaleRightDepth);
	  printf("    Left Curve  (LSLC): %s\n", Curve(op.scaleLeftCurve));
	  printf("    Right Curve (LSrC): %s\n", Curve(op.scaleRightCurve));
	  printf("  Keyboard Rate Scaling (OrS): %u\n", op.rateScale);
	  printf("  AM Sensitivity        (ANS): %u\n",
		 op.amplitudeModulationSensitivity);
	  printf("  Key Velocity Sensitivity (KUS): %u\n",
		 op.keyVelocitySensitivity);
	  printf("  Output Level (OLUL): %u\n", op.outputLevel);
	  printf("  Oscillator Mode(OSCN): %s\n", Mode(op.oscillatorMode));
	  // If ratio mode
	  if (op.oscillatorMode == 0)
	    {
	      double coarse = op.frequencyCoarse;
	  ...
	  printf ("  | Freq Coarse (FrEC): %d\n", op.frequencyCoarse);
	  printf ("  + Freq Fine   (FrEC): %d\n", op.frequencyFine);
	  printf("  Detune (dEtu): %d (%d)\n", op.detune, op.detune - 7);
	}
      printf("-------------------------------------------------\n");
    }
}
